...
{
	_LTE_USART->TDR = ch;
	while(!(_LTE_USART->ISR&USART_ISR_TXE))
	{;}
	return ch;
}
//##################################################################################################################
...
{
	LL_DMA_SetPeriphAddress(DMA1, LL_DMA_CHANNEL_3, (uint32_t)&_LTE_USART->RDR);
	LL_DMA_SetMemoryAddress(DMA1, LL_DMA_CHANNEL_3, (uint32_t)lte_usart_rx_dma_buffer);
	LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_3, ARRAY_LEN(lte_usart_rx_dma_buffer));
...
void lte_usart_rx_check(void) {
    ...
    pos = ARRAY_LEN(lte_usart_rx_dma_buffer) - LL_DMA_GetDataLength(DMA1, LL_DMA_CHANNEL_3);
